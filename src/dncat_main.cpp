...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 6; i++) prev_motion[i] = 0.0;
  /*
  ** -------------------
  ** READ COMMAND LINE :
  ** -------------------
  ...
  */
  ...
  subvxl_vol = (pixel_width * pixel_width * slice_width) /
               (subvoxel_index * subvoxel_index * subvoxel_index);
  ...
  */
  ...
  zoff = 0.0;
  centerx = 93.0;
  centery = 106.0;  // as determined from skull in Rhino3d
  ...
  for(i = 0; i < 100; i++)
    ...
    for(j = 0; j < 6; j++)
      {
      C = CurvePoint(motion_curve[j].pol.n-1, 3, motion_curve[j].knt,
  motion_curve[j].pol.Pw, (float)i/100.0);
      printf(" %f", C.y);
      }
    }
  exit(1);
  ...
  */
  for (i = 0; i <= XDIM * YDIM * ZDIM_OUTPUT - 1; i++) fphan_avg[i] = 0.;
  ...
  if (motion_option == 0) {
    C = CurvePoint(motion_curve[0].pol.n - 1, 3, motion_curve[0].knt,
                   motion_curve[0].pol.Pw, motion_start_ph_index);
    xr = C.y * x_rot;

    C = CurvePoint(motion_curve[1].pol.n - 1, 3, motion_curve[1].knt,
                   motion_curve[1].pol.Pw, motion_start_ph_index);
    yr = C.y * y_rot;

    C = CurvePoint(motion_curve[2].pol.n - 1, 3, motion_curve[2].knt,
                   motion_curve[2].pol.Pw, motion_start_ph_index);
    zr = C.y * z_rot;

    C = CurvePoint(motion_curve[3].pol.n - 1, 3, motion_curve[3].knt,
                   motion_curve[3].pol.Pw, motion_start_ph_index);
    xt = C.y * x_trans;

    C = CurvePoint(motion_curve[4].pol.n - 1, 3, motion_curve[4].knt,
                   motion_curve[4].pol.Pw, motion_start_ph_index);
    yt = C.y * y_trans;

    C = CurvePoint(motion_curve[5].pol.n - 1, 3, motion_curve[5].knt,
                   motion_curve[5].pol.Pw, motion_start_ph_index);
    zt = C.y * z_trans;
  ...
  for (kf = 1; kf <= out_frames; kf++) {
    ...
    if (motion_option == 1) {
      motion_phase_index =
          motion_start_ph_index + motion_incr_ph_index * (kf - 1);
      while (motion_phase_index >= 1.000)
        motion_phase_index = motion_phase_index - 1;

      C = CurvePoint(motion_curve[0].pol.n - 1, 3, motion_curve[0].knt,
                     motion_curve[0].pol.Pw, motion_phase_index);
      xr = (C.y - prev_motion[0]) * x_rot;
      prev_motion[0] = C.y;
      printf("\nMOTION for frame %i, time = %f: angx = %f ", kf, C.x,
             C.y * x_rot);

      C = CurvePoint(motion_curve[1].pol.n - 1, 3, motion_curve[1].knt,
                     motion_curve[1].pol.Pw, motion_phase_index);
      yr = (C.y - prev_motion[1]) * y_rot;
      prev_motion[1] = C.y;
      printf("angy = %f ", C.y * y_rot);

      C = CurvePoint(motion_curve[2].pol.n - 1, 3, motion_curve[2].knt,
                     motion_curve[2].pol.Pw, motion_phase_index);
      zr = (C.y - prev_motion[2]) * z_rot;
      prev_motion[2] = C.y;
      printf("angz = %f ", C.y * z_rot);

      C = CurvePoint(motion_curve[3].pol.n - 1, 3, motion_curve[3].knt,
                     motion_curve[3].pol.Pw, motion_phase_index);
      xt = (C.y - prev_motion[3]) * x_trans;
      prev_motion[3] = C.y;
      printf("tx = %f ", C.y * x_trans);

      C = CurvePoint(motion_curve[4].pol.n - 1, 3, motion_curve[4].knt,
                     motion_curve[4].pol.Pw, motion_phase_index);
      yt = (C.y - prev_motion[4]) * y_trans;
      prev_motion[4] = C.y;
      printf("ty = %f ", C.y * y_trans);

      C = CurvePoint(motion_curve[5].pol.n - 1, 3, motion_curve[5].knt,
                     motion_curve[5].pol.Pw, motion_phase_index);
    ...
    */
    for (j = 0; j < tzdim; j++) {
      ...
      for (i = 0; i < txdim * tydim; i++) slice[i] = -1.0;
      ...
      for (k = 0; k <= END_MODELS; k++) {
        if (k != 21 && k != 28 && k != 255) {
          ...
          for (i = 0; i < tmodel[k].num_tris; i++) {
            if (Test_extents_tri_z(tmodel[k].tris[i], j)) {
              slice_tmodel.tris[num].vertex[0].x =
                  tmodel[k].tris[i].vertex[0].x;
              slice_tmodel.tris[num].vertex[0].y =
                  tmodel[k].tris[i].vertex[0].y;
              slice_tmodel.tris[num].vertex[0].z =
                  tmodel[k].tris[i].vertex[0].z;

              slice_tmodel.tris[num].vertex[1].x =
                  tmodel[k].tris[i].vertex[1].x;
              slice_tmodel.tris[num].vertex[1].y =
                  tmodel[k].tris[i].vertex[1].y;
              slice_tmodel.tris[num].vertex[1].z =
                  tmodel[k].tris[i].vertex[1].z;

              slice_tmodel.tris[num].vertex[2].x =
                  tmodel[k].tris[i].vertex[2].x;
              slice_tmodel.tris[num].vertex[2].y =
                  tmodel[k].tris[i].vertex[2].y;
              slice_tmodel.tris[num].vertex[2].z =
                  tmodel[k].tris[i].vertex[2].z;
      ...
      for (k = xoffset; k < txdim; k++)
        for (l = yoffset; l < tydim; l++) {
          index1 = (int)((k - xoffset) / subvoxel_index) +
                   (int)((l - yoffset) / subvoxel_index) * XDIM +
                   (int)(j / subvoxel_index) * XDIM * YDIM;
          index2 = k + l * txdim;
          ...
          if (slice[index2] != -1) {
            ...
            fphan_id[index1] +=
                ...
                (subvoxel_index * subvoxel_index * subvoxel_index);
            fphan[index1] +=
                activ / (subvoxel_index * subvoxel_index * subvoxel_index);
            fphan_avg[index1] += activ / (subvoxel_index * subvoxel_index *
                                          subvoxel_index * out_frames);
          }
        }
    } /*End slice loop*/
    ...
    //  GetVolumes(fphan_id);
    if (act_phan_each == 1) { /* SAVE ACTIVITY PHANTOM for frame kf to file */
  ...
  /*
      ...
      --------------------------
  */
  if (act_phan_ave == 1) { /* SAVE AVERAGE ACTIVITY PHANTOM to file */
